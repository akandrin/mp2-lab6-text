...
void TText::PrintRec(std::ostream & os, TNode * p, int level_count, int& line_number, bool print_string_number) const
{
	...
	if (p != nullptr) {
		if (!is_head)
			os << std::endl;
		if (!print_string_number)
			os << line_number++ << ' ';
		os << std::string(space_count, ' ') << p->str;
		if (p->pDown != nullptr)
			os << std::endl << std::string(space_count + 2, ' ') << '{';
		PrintRec(os, p->pDown, level_count + 1, line_number, print_string_number);
		if (p->pDown != nullptr)
			os << std::endl << std::string(space_count + 2, ' ') << '}';
		PrintRec(os, p->pNext, level_count, line_number, print_string_number);
	}
}
